...
BeforeAll {
    ...
    function ThenUseApprovedVerbs
    {
        ...
        $verbs =
            Get-Command -Module 'Carbon.ScheduledTasks'|
...
Describe 'Carbon.ScheduledTasks' {
    It 'should have about help topic' {
        GivenModuleImported
        ThenHelpTopic 'about_Carbon.ScheduledTasks' -Exists
    }
    ...
    It 'should have a help topic for each command' {
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'Carbon.ScheduledTasks' -CommandType Function,Cmdlet,Filter))
        {
